 ...
 * Description: Gestiona shortcodes personalizados y variables personalizadas con compatibilidad para emails
 * Version: 1.2.0
 * Author: Tu Nombre
...
class CustomShortcodesManager {
    ...
    public function activate() {
        ...
        $default_shortcodes = array(
            array(
                'name' => 'boton',
                'content' => '<a href="{url}" class="btn {clase}" style="background-color:{color}; color:white; padding:10px 20px; text-decoration:none; display:inline-block; border-radius:4px;">{texto}</a>',
                'description' => 'Botón personalizable',
                'created' => current_time('mysql')
            ),
            array(
                'name' => 'panel',
                'content' => '<div class="panel {tipo}" style="border:1px solid #ddd; padding:15px; margin:10px 0; background-color:#f9f9f9;"><h3 style="margin-top:0;">{titulo}</h3><div class="panel-content">{{content}}</div></div>',
                'description' => 'Panel con contenido',
                'created' => current_time('mysql')
            ),
            array(
                'name' => 'firma_email',
                'content' => '<div style="border-top:1px solid #ccc; padding-top:15px; margin-top:20px; color:#666; font-size:12px;">Saludos,<br>{nombre}<br>{cargo}<br>{empresa}</div>',
        ...
        $default_integration = array(
            ...
            'allowed_shortcodes' => array('boton', 'panel', 'firma_email')
        );
    ...
    private function init_form_integrations() {
        ...
        if ($integration_settings['cf7_enabled'] && $integration_settings['process_in_emails']) {
            // Integración con Contact Form 7
            add_filter('wpcf7_mail_components', array($this, 'process_cf7_email_shortcodes'), 10, 3);
            
            // Integración con otros formularios (genérico)
            add_filter('wp_mail', array($this, 'process_generic_email_shortcodes'));
            
            // Procesar shortcodes en los campos de CF7
            add_filter('wpcf7_form_elements', 'do_shortcode');
        }
    }
    
    public function process_cf7_email_shortcodes($components, $wpcf7, $mail = null) {
        // Procesar shortcodes en el asunto, cuerpo y headers del email
        if (isset($components['subject'])) {
            $components['subject'] = $this->process_email_shortcodes($components['subject']);
        }
        
        if (isset($components['body'])) {
            $components['body'] = $this->process_email_shortcodes($components['body']);
        }
        
        if (isset($components['additional_headers'])) {
            $components['additional_headers'] = $this->process_email_shortcodes($components['additional_headers']);
        }
    ...
    private function process_email_shortcodes($content) {
        // Primero procesar shortcodes de WordPress
        $content = do_shortcode($content);
        
        // Luego procesar nuestros shortcodes personalizados
        $saved_shortcodes = get_option($this->shortcodes_option, array());
        $integration_settings = get_option($this->cf7_integration_option, array());
        $allowed_shortcodes = $integration_settings['allowed_shortcodes'];
        
        foreach ($saved_shortcodes as $shortcode) {
            if (in_array($shortcode['name'], $allowed_shortcodes)) {
                // Buscar y reemplazar shortcodes en el contenido
                $pattern = '/\[' . $shortcode['name'] . '(.*?)\](.*?)\[\/' . $shortcode['name'] . '\]|\[(' . $shortcode['name'] . ')(.*?)\]/';
                
                $content = preg_replace_callback($pattern, function($matches) use ($shortcode) {
                    return $this->render_email_shortcode($shortcode, $matches);
                }, $content);
            }
        }
    ...
    private function render_email_shortcode($shortcode_data, $matches) {
        // Determinar si es shortcode con contenido o autoclosing
        if (isset($matches[2]) && $matches[2] !== '') {
            // Shortcode con contenido: [shortcode]contenido[/shortcode]
            $content = $matches[2];
            $atts_string = $matches[1];
        } else {
            // Shortcode autoclosing: [shortcode atributo="valor"]
            $content = null;
            $atts_string = isset($matches[4]) ? $matches[4] : '';
        }
        
        // Parsear atributos
        $atts = array();
        if (!empty($atts_string)) {
            preg_match_all('/(\w+)=["\']([^"\']*)["\']/', $atts_string, $attribute_matches, PREG_SET_ORDER);
            foreach ($attribute_matches as $attr) {
                $atts[$attr[1]] = $attr[2];
            }
        }
        
        return $this->render_shortcode($shortcode_data, $atts, $content);
    }
    ...
    public function email_settings_page() {
        ...
        <div class="wrap">
                    ...
                    <tr>
                        ...
                        <td>
                            <fieldset>
                                ...
                                <?php foreach ($shortcode_names as $name): ?>
                                <label>
                            ...
                            <p class="description">
                                <?php _e('Selecciona qué shortcodes quieres que se procesen en los emails. No selecciones shortcodes que puedan causar problemas de seguridad.', 'custom-shortcodes'); ?>
                            </p>
                        </td>
                    </tr>
                </table>
            ...
            <div class="email-integration-info">
                <h3><?php _e('Uso en Contact Form 7', 'custom-shortcodes'); ?></h3>
                <p><?php _e('Para usar shortcodes en Contact Form 7:', 'custom-shortcodes'); ?></p>
                <ol>
                    ...
                    <li><?php _e('Ejemplo en el cuerpo del email:', 'custom-shortcodes'); ?></li>
                </ol>
...
[firma_email nombre="Soporte" cargo="Equipo de Soporte" empresa="[nombre_sitio]"]

Puedes contactarnos en cualquier momento:
[boton url="[url_sitio]/contacto" texto="Página de Contacto" color="#0073aa"]

Saludos,
El equipo de [nombre_sitio]</code></pre>
            </div>
        </div>
        <?php
    }
    ...
    public function documentation_page() {
        ...
        <div class="wrap">
            ...
            <div class="documentation-content">
                <h2><?php _e('Shortcodes para Emails', 'custom-shortcodes'); ?></h2>
                ...
                <h3><?php _e('Uso en Contact Form 7:', 'custom-shortcodes'); ?></h3>
                <p><?php _e('Los shortcodes personalizados funcionan en:', 'custom-shortcodes'); ?></p>
                <ul>
                    <li><?php _e('Asunto del email', 'custom-shortcodes'); ?></li>
                    <li><?php _e('Cuerpo del email', 'custom-shortcodes'); ?></li>
                    <li><?php _e('Email de confirmación', 'custom-shortcodes'); ?></li>
                    <li><?php _e('Headers adicionales', 'custom-shortcodes'); ?></li>
                </ul>
                
                <h3><?php _e('Ejemplo Completo para CF7:', 'custom-shortcodes'); ?></h3>
                <pre><code>Asunto: Nuevo mensaje de contacto - [fecha_actual]
...
[panel tipo="info" titulo="Información de Contacto"]
Este mensaje fue enviado el [fecha_actual format="d/m/Y H:i"] 
desde el sitio [nombre_sitio].
[/panel]

[firma_email nombre="Sistema Automático" cargo="Notificaciones" empresa="[nombre_sitio]"]</code></pre>
                
                <h2><?php _e('Shortcodes para Custom Fields', 'custom-shortcodes'); ?></h2>
                <!-- ... (mantener el contenido anterior de documentación) ... -->
            </div>
        </div>
        <?php
    }
